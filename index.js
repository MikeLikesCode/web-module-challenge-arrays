...
function is31Flavors(flavors){
   return flavors.length === 31 ? true : false
}
...
function addFlavor(flavors , addFlavor){
...
function removeLastFlavor(flavors, removeFlavor){
   flavors.pop();
   return flavors
}
...
function filterByWord(flavors,flavor){
    ...
    for(let i = 0; i < flavors.length; i++){
        if(flavors[i].includes(flavor)){
           results.push(flavors[i])
        }
    }
...
function getAverageWordLength(flavors){
    ...
    for(let i = 0; i < flavors.length; i++){
         average.push(flavors[i].split(' ').length);
    }
    ...
    grandTotal = Math.round(total/average.length);
